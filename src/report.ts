...
export function printCliReport(result: AuditResult): void {
  ...
  console.log(pc.dim(`Agent: ${result.agentModel} | ${result.timestamp}\n`))
...
function findingHtml(f: Finding): string {
  ...
  return `
    <div class="finding" style="border-left:3px solid ${color}">
      ...
      <div style="margin-bottom:0.5rem">${esc(f.why_this_matters)}</div>
      <div class="detail">
...
export function writeHtmlReport(result: AuditResult, dir: string): string {
  ...
  const failures = result.questions.filter(q => !q.skipped && !q.pass)
  const totalQuestions = result.passed + result.failed + result.skipped
...
<head>
  ...
  <title>AI Audit: ${esc(result.domain)}</title>
  <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;700&family=Inter:wght@400;500;600&display=swap" rel="stylesheet">
  <style>
    :root { --bg:#09090b; --fg:#fafafa; --border:#1e1e23; --surface:#060608; --muted:#9f9fa9; --green:#9ece6a; --red:#f7768e; --yellow:#e0af68; --purple:#bb9af7 }
    * { margin:0; padding:0; box-sizing:border-box }
    body { background:var(--bg); color:var(--fg); font-family:Inter,system-ui,sans-serif; line-height:1.6; padding:1rem; max-width:900px; margin:0 auto }
    ...
    .scores span { font-size:1.5rem; font-weight:700 }
    ...
    .fail-header { margin-bottom:1rem; word-break:break-word }
    .fail-header .label { color:var(--red); font-weight:600 }
    ...
    .answer-box .text { color:#ccc; word-break:break-word }
    .finding { margin-bottom:1rem; padding:1rem; background:#0d0d10 }
    ...
    .footer { margin-top:2rem; padding-top:1rem; border-top:1px solid var(--border); color:var(--muted); font-size:0.8rem; line-height:2 }
    @media (max-width:600px) {
      body { padding:0.75rem }
      h1 { font-size:1.2rem }
      .scores span { font-size:1.2rem }
...
<body>
  <div style="margin-bottom:2rem">
    <h1>AI Audit: ${esc(result.domain)}</h1>
    <div class="scores">
      <span style="color:var(--green)">${result.passed} passed</span>
      <span style="color:${result.failed > 0 ? 'var(--red)' : 'var(--muted)'}">${result.failed} failed</span>
    </div>
    <p class="meta">${totalFindings} issues across ${totalQuestions} questions &middot; Agent: ${esc(result.agentModel)}</p>
  ...
  ${result.questions.filter(q => !q.skipped && q.pass).map(qa => `
    <div class="pass-row">
      <span class="label">PASS</span>
      <span class="question">${esc(qa.question)}</span>
    </div>`).join('\n')}

  ${failures.length > 0 ? failures.map(questionHtml).join('\n') : ''}

  <div class="footer">
    Generated by <a href="https://github.com/bradxyz/agent-audit">agent-audit</a><br>
    Is this your company? <a href="https://agentbase.to/a/${esc(slug)}">Fix what AI agents say about you &rarr;</a>
  </div>
</body>
</html>`
